/**
 * LP8-I5e MAA HTTP sidecar — isolated MAA 0.21.0 / schema 0018 with publication bridge.
...
async function main(): Promise<void> {
  if (SERVICE_VERSION !== "0.21.0") {
    throw new Error(`MAA sidecar expects 0.21.0, got ${SERVICE_VERSION}`);
  }
  if (CURRENT_DATABASE_SCHEMA_VERSION !== "0018") {
    throw new Error(`MAA sidecar expects schema 0018, got ${CURRENT_DATABASE_SCHEMA_VERSION}`);
  }
  ...
  const raw = ConfigSchema.parse({
    ...
    MAA_LEARNING_PLANE_PUBLICATION_SUBMIT_ENABLED: "true",
    MAA_LEARNING_PLANE_PUBLICATION_RECONCILE_ENABLED: "true",
    MAA_LEARNING_PLANE_DISCOVERY_ENABLED: "true",
    MAA_LEARNING_PLANE_PACKAGE_FETCH_ENABLED: "true",
    MAA_LEARNING_PLANE_LOCAL_REFERENCE_ENABLED: "true",
    MAA_LEARNING_PLANE_LOCAL_REFERENCE_REVIEW_ENABLED: "true",
    MAA_LEARNING_PLANE_EXTERNAL_RETRIEVAL_ENABLED: "true",
    MAA_LEARNING_PLANE_REFERENCE_RECEIPT_ENABLED: "true",
    MAA_LEARNING_PLANE_USE_RECEIPT_ENABLED: "true",
    MAA_LEARNING_PLANE_INFLUENCE_RECEIPT_ENABLED: "true",
    MAA_LEARNING_PLANE_CHALLENGE_ENABLED: "true",
    MAA_LEARNING_PLANE_PK_LIFECYCLE_RECONCILE_ENABLED: "true",
    MAA_LEARNING_PLANE_OFFLINE_GRACE_HOURS: "24"
  });
        ...
        try {
          ...
          if (req.method === "GET" && url.pathname === "/uat/sql-count") {
            ...
            const allowed = new Set([
              ...
              "lp_pk_publication_proposals",
              "lp_pk_local_references",
              "lp_pk_use_traces",
              "lp_pk_influence_traces",
              "lp_pk_challenges",
              "lp_pk_outbox",
              "lp_pk_discovery_queries",
              "memory_items",
              "error_book_entries",
              "memory_proposals"
            ]);
          ...
          if (req.method === "POST" && url.pathname === "/uat/seed-reviewed-memory") {
            const bodyText = Buffer.concat(chunks).toString("utf8");
            const body = bodyText ? (JSON.parse(bodyText) as Record<string, unknown>) : {};
            const now = new Date().toISOString();
            const memoryId = String(body.memoryId ?? `mem_i5e_${Date.now()}`);
            container.repos.memoryItems.insert({
              memoryId,
              memoryType: String(body.memoryType ?? "capability_note"),
              authorityStatus: "reviewed_project",
              title: String(body.title ?? "I5e capability limitation"),
              statement: String(
                body.statement ??
                  "Unsupported sponsored filter cannot be treated as authoritative ranking evidence."
              ),
              summary: String(body.summary ?? "Bounded capability limitation for I5e UAT."),
              ...
              payloadJson: JSON.stringify({ uat: true, projectId: body.projectId ?? "proj_i5e" }),
              createdAt: now,
              updatedAt: now
            });
            return send(201, { memoryId });
          }
          if (req.method === "POST" && url.pathname === "/uat/pk-outbox-tick") {
            const worker = container.learningPlane?.publishedKnowledgeOutboxWorker;
            if (!worker) return send(500, { error: "pk outbox worker missing" });
            await worker.tick();
            return send(200, { ticked: true });
          }
